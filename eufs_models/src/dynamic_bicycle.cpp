...
State DynamicBicycle::_f(const State &x, const Input &u, const double Fx, const double FyF, const double FyR) {
    ...
    x_dot.r_z =
        (std::cos(u.delta) * FyF_tot * _param.kinematic.l_F - FyR_tot * _param.kinematic.l_R) / _param.inertia.I_z;

    return x_dot;
}

State DynamicBicycle::_fKinCorrection(const State &x_in, const State &x_state, const Input &u, const double Fx,
                                      const double dt) {
    State x = x_in;
...
double DynamicBicycle::_getNormalForce(const State &x) { return _param.inertia.g * _param.inertia.m + _getFdown(x); }
