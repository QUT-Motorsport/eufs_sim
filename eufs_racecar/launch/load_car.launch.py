...
def spawn_car(context, *args, **kwargs):
    ...
    noise_file = join(
        get_package_share_directory("eufs_models"), "config", "noise.yaml"
    )
    recolor_config = join(
        get_package_share_directory("eufs_plugins"), "config", "cone_recolor.yaml"
    )
    bounding_boxes_file = os.path.join(
        get_package_share_directory("eufs_plugins"), "config", "boundingBoxes.yaml"
    )
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "x", default_value="0", description="Vehicle initial x position"
            ),
            DeclareLaunchArgument(
                "y", default_value="0", description="Vehicle initial y position"
            ),
            DeclareLaunchArgument(
                "z", default_value="0", description="Vehicle initial z position"
            ),
            DeclareLaunchArgument(
                "roll", default_value="0", description="Vehicle initial roll"
            ),
            DeclareLaunchArgument(
                "pitch", default_value="0", description="Vehicle initial pitch"
            ),
            DeclareLaunchArgument(
