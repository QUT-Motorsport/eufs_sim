...
int get_cone_colour(
    gazebo::physics::LinkPtr cone_link,
    ...
    if (logger) {
        RCLCPP_WARN_ONCE(*logger, "Cannot get cone type from link in the track model in simulation: %s", link_name.c_str());
    }
...
driverless_msgs::msg::ConeWithCovariance get_cone_from_link(
    gazebo::physics::LinkPtr cone_link,
...
driverless_msgs::msg::ConeDetectionStamped get_ground_truth_track(
    ...
    for (auto const &link : links) {
        track.cones_with_cov.push_back(
            get_cone_from_link(link, {0, 0, 0, 0}, logger)
        );
    }
