...
def generate_bezier_track(start_point):
                ...
                for g in range(1,len(goal_points)):
                        bezier_out,tangent_in,prev_tangent = get_random_bezier(
                                goal_points[g-1],
                                goal_points[g],
                ...
                bezier_out,tangent_in,prev_tangent = get_random_bezier(
                        ...
                        start_point,
                        start_tangent=prev_tangent,
                        calculate_tangent_angle=initial_tangent
                )
                xys.extend([bezier_out(t*0.01) for t in range(0,101)])
                
                return convert_points_to_all_positive(xys)

def get_random_bezier(start_point, 
                      end_point, 
                      start_tangent = None, 
                      calculate_tangent_angle = None):
                """
                Generates a random cubic bezier, and returns it as a 
                function of t so that arbitrary precision can be used.

                Returns a tuple of the bezier function, the incoming tangent, 
                and the outgoing tangent.

                Requires the start and end point of the bezier to be specified
                Optional parameters are:

                        start_tangent: the tangent that the start of the bezier curve 
                                       should follow

                        calculate_tangent_angle: the tangent that the end of 
                                                 the bezier curve should follow
                Optional parameters are randomized if not specified.
                """

                # For the math to work out, we need Beziers to be 
                # [at least] cubic (parameterized by 4 points)
                if start_tangent is None:
                        start_tangent = uniform(0, 2 * math.pi)
                if calculate_tangent_angle is None:
                        calculate_tangent_angle = uniform(0, 2 * math.pi)
                ...
                scale = uniform(10, 100)
                pn_1_to_pn = (math.cos(calculate_tangent_angle) * scale,
                              math.sin(calculate_tangent_angle) * scale)
                pn = end_point
                pn_1 = (pn[0] - pn_1_to_pn[0], pn[1] - pn_1_to_pn[1])

                control_points = [p0, p1, pn_1, pn]

                return (get_parametric_bezier(control_points),
                        start_tangent,
...
def generate_autocross_trackdrive_track(start_point):
                ...
                (generated, cur_point, delta_length) = generate_straight(
                        start_point,
                ...
                # Let's generate towards each goal point
                for goal_point in goal_points:
                        prev_xys = xys
                        (generated, cur_point, length) = generate_path_from_point_to_point(
                                cur_point,
                                goal_point,
                ...
                outer_turn_angle = math.acos(
                        - initial_tangent[0] * goal_tangent[0] 
                        - initial_tangent[1] * goal_tangent[1]  
                )
                ...
                straight_length = magnitude((xys[-1][0] - start_point[0], 
                                             xys[-1][1] - start_point[1])) * 1.1
                (generated, cur_point, delta_length) = generate_straight(
                        cur_point, 
                        straight_length,
                        calculate_tangent_angle(xys)
...
def generate_path_from_point_to_point(
                                start_point,
                                end_point,
                                tangent_in,
                                depth=20,
        ...
        [And hairpined parameter prevents multiple hairpins - 
          we should have at most one or else its hard to generate nice paths]
        [many_hairpins overrides this and allows an arbitrary amount]
        [fuzz_radius is how close to the end we want to be,
          0 will make it arrive exactly in place.]
        """
        ...
        # We want to know ahead of time which way the circle will turn - 
        # that way we don't get loopty-loops
        # Where the circle turns nearly all the way around when it would have been 
        # better to have the center on the other side.
        ...
        # Our circle code takes in "turn_against_normal" - alternatively, 
        # this parameter is equivalent to passing in
        # a vector pointing TOWARDS the radius.  So let's calculate that!  
        # If we have the goal be at a "higher" angle
        # than it, we want the radius to be up, otherwise down.  
        # The angle of it pointing up is simply pi/2 higher than the tangent (90 degrees)
        normal_angle = cap_angle(math.pi / 2 + tangent_in)

        #We now calculate if we need to add an additional pi to it.
        #If direct_path_angle is higher than tangent_in - but how do we define higher?  
        # We say that it is higher if the
        #counterclockwise angle difference is smaller than the clockwise difference.
        if (cap_angle(direct_path_angle - normal_angle) 
          > cap_angle(direct_path_angle - direct_path_angle)):
        ...
        (generated, cur_point, delta_length, output_normal) = (
        ...
        if square_distance <= TrackGenerator.MAX_STRAIGHT**2 + fuzz_radius**2:
                ...
                # Go as far as we can (unless we're very close in which case don't, 
                # because it'll cause the next iteration to look weird)
                if square_distance <= (TrackGenerator.MAX_STRAIGHT * 1.2)**2:
                        straight_size = TrackGenerator.MAX_STRAIGHT  
                else: 
                        straight_size = TrackGenerator.MAX_STRAIGHT/2
                (generated, cur_point,delta_length) = generate_straight(
                        points[-1],
                        straight_size,
                        calculate_tangent_angle(points)
                ...
                if make_cturn or (hairpined and not many_hairpins):
                        (generated, cur_point, delta_length, output_normal) = (
                                generate_constant_turn(
                                        ...
                                        turn_against_normal = output_normal
                                )
                        )
                        length += delta_length
                        points.extend(generated)
                ...
                # Make a hairpin
                        ...
                        (generated, cur_point,delta_length) = generate_hairpin_turn(
                                cur_point,
                                uniform(TrackGenerator.MIN_HAIRPIN,TrackGenerator.MAX_HAIRPIN),
                                calculate_tangent_angle(points),
                                switchback_num=num_switches
                        )
                        length += delta_length
                        points.extend(generated)
...
def generate_hairpin_turn(
        ...
        wobbliness: The percentage of circumference of the circles.  
                     If left blank, will be between 0.45 and 0.55
        ...
        uniform_circles: If True, all circles will be of the same radius, 
                         otherwise their radii is random.
        """
        cur_point = start_point
        ...
        if switchback_num is None:
                switchback_num = 2 * randrange(
                        TrackGenerator.MIN_HAIRPIN_NUM,TrackGenerator.MAX_HAIRPIN_NUM
                )
        if wobbliness is None:
                wobbliness = uniform(0.45, 0.55)
        if straight_size is None:
                straight_size = uniform(TrackGenerator.MIN_STRAIGHT,TrackGenerator.MAX_STRAIGHT)
        ...
        for a in range(0, switchback_num):
                ...
                #cturn
                (generated, cur_point,delta_length,last_normal) = generate_constant_turn(
                        cur_point,
                        circle_size,
                        cur_tangent,
...
def generate_constant_turn_until_facing_point(start_point,radius,tangent_in,goal_point,turn_left=None,turn_against_normal=None,recursed=False):
        """
        This is a split-off version of generate_constant_turn, 
        where we give it a direction we want it to stop at.
        """
        ...
        angle = 2*math.pi
        flipper = -1 if tangent_in*pure_x > 0 else 1 
        ...
        if turn_against_normal != None:
                ...
                # So first we want to compute the current normal 
                # to make sure we don't change anything
                ...
                # Due to floating point stuffs, we won't check direct equality, 
                # we'll just look at the sign!
                # We want normals to be flipped, so its bad if cur_normal has the same sign!
                # We also only need to check both components just in the case 
                # where the normal is (0,y)
                need_to_flip = (abs(cur_normal[0] - turn_against_normal[0]) < 0.1 
                             or abs(cur_normal[1] - turn_against_normal[1]) < 0.1)
        ...
        for t in range(0,max_range):
                ...
                if t != 0:
                        ...
                        if t > 0.5 * max_range and not recursed:
                                ...
                                if turn_against_normal is None:
                                        new_turn_against_normal = None
                                else: 
                                        new_turn_against_normal = normalize_vec((
                                                -turn_against_normal[1],
                                                 turn_against_normal[0]
                                        ))
                                return generate_constant_turn_until_facing_point(
                                        ...
                                        goal_point,
                                        turn_left = turn_left,
                                        turn_against_normal = new_turn_against_normal,
                                        recursed = True
                                )
                        if abs(
        ...
        # Now we want to find the normal vector, 
        # because it's useful to have to determine whether it curves inwards or outwards
...
def generate_constant_turn(
                           start_point,
                           radius,
                           tangent_in,
                           turn_left=None,
                           circle_percent=None,
                           turn_against_normal=None):
        """
        ...
        if turn_against_normal is not None:
        ...
        # calculates if turning clockwise (-1) or anticlockwise (1)
        cross_product = np.cross([tangent_vec[0], tangent_vec[1]], [normal_vec[0], normal_vec[1]])
        flipper = -1 if cross_product < 0 else 1
        ...
        # Now we want to find the normal vector, because it's 
        # useful to have to determine whether it curves inwards or outwards
        # Normal vectors are always parallel to the vector from center to end point
...
def generate_straight(start_point,length,angle):
        ...
        # However we actually don't want that because it will mess
        # with the self-intersection-detection
        # later on.
        scale_factor = 10.0
        if tmax >= 0:
                points = [(t / scale_factor + startx, slope * t / scale_factor + starty) 
                          for t in range(0, int(scale_factor * math.ceil(tmax)))]
        else:
                points = [(-t / scale_factor + startx, -slope * t / scale_factor + starty) 
                          for t in range(0,int(scale_factor*math.ceil(-tmax)))]
