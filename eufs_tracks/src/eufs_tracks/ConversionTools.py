...
sys.path.insert(1, os.path.join(get_package_share_directory('eufs_tracks'),
                                'csv'))  # nopep8
from .track_gen import Track  # noqa: E402
from .TrackGenerator import (  # noqa: E402
...
# treat it as wanting the .launch)
#                (since the end user shouldn't have to care about the
#                distinction)
# .png
# .csv
# raw_data ("xys",this will be hidden from the user as it is only used to
# convert into .pngs)
...
class ConversionTools(Node):
    ...
    # Double cone closeness, how close do we place the doubled-up starting
    # cones?
    ...
    def get_metadata_pixel(x, y, corner, pixels, size):
        ...
        x_out, y_out = ConversionTools.get_metadata_pixel_location(x, y,
                                                                   corner,
                                                                   size)
    ...
    def get_metadata_pixel_location(x, y, corner, size):
        """Returns the metadata pixel (x,y,corner)'s location on the actual
        image."""
        width = size[0]
        height = size[1]
    ...
    def convert_scale_metadata(pixel_values):
        ...
        if (primary_pixel == (255, 255, 255, 255)
                and secondary_pixel == (255, 255, 255, 255)):
            return 1
    ...
    def deconvert_scale_metadata(data):
        ...
        First in list is the primary metadata pixel, second in list is the
        secondary
        """
    ...
    def deconvert_version_metadata(data):
        """This function is the reverse transformation as
    ...
    def convert(cfrom, cto, which_file, params={}, conversion_suffix="",
        ...
        cfrom:      Type to convert from (xys, png, launch, csv) [should be
        a string]
        cto:        Type to convert to   (png, launch, csv)      [should be
        a string]

        which_file: The file to be converted - should be a full filepath.

        params:     Additional parameters that may be necessary.
                    These will depend on conversion type, so check the
                    docstrings of
                    the specific desired conversion function for full
                    information.

        conversion_suffix: A suffix appended to the filename of the output
        file.

        If converted to an image, it will return that image in case the
        function caller
    ...
    def comps_to_csv(which_file, params, conversion_suffix="",
        ...
        conversion_suffix:     Additional suffix to append to the output
        filename.
        ...
        for cone in cone_locs:
            ...
            if color == CONE_START:
                direction_vector = (
                    math.cos(
                        angle) * 0.5 * ConversionTools.DOUBLE_CONE_CLOSENESS,
                    math.sin(
                        angle) * 0.5 * ConversionTools.DOUBLE_CONE_CLOSENESS
                )
                new_cone1 = (
        ...
        # Add noise pixels
        im_to_display = ConversionTools.comps_to_png(GENERATED_FILENAME,
                                                     params)
        ...
        # Save it
        output = OrderedDict([
            ("tag", [name_from_color(color) for px, py, color in cone_locs] + ["car_start"]),
    ...
    def comps_to_png(which_file, params, conversion_suffix="",
        ...
        # Now we want to make all pixels bordering the track become magenta
        # (255,0,255) -
        ...
        for px, py, color in cone_locs:
            ...
            if (not is_track(
                    pixels[int(px), int(py)]) or color is CONE_ORANGE):
    ...
    def xys_to_png(which_file, params, conversion_suffix="",
        ...
        which_file:            Output filename
        params["point list"]:  Tuple of: list of points, image width,
        image height
    ...
    def png_to_launch(which_file, params, conversion_suffix="",
                      override_name=None):
        """
        Converts a .png to a .launch with .world, model.sdf, model.config
        files.
        ...
        loc = ConversionTools.get_metadata_pixel_location(  # noqa: F841
            4,
            4,
            ConversionTools.BOTTOM_RIGHT,
            im.size
        ...
        launch_merged = GENERATED_FILENAME.join(
            launch_merged.split("%FILLNAME%"))

        # These are helper functions to convert pixel data, such as
        # coordinates,
        ...
        world_merged = GENERATED_FILENAME.join(
            world_merged.split("%FILLNAME%"))
        ...
        config_merged = GENERATED_FILENAME.join(
            config_merged.split("%FILLNAME%"))
        ...
        #            a low-complexity collision to prevent falling out the
        #            world
        ...
        sdf_split_along_collisions = sdf_split_again[6].split(
            "%FILLCOLLISION%")
        sdf_split_along_ghost_collisions = sdf_split_again[6].split(
            "%FILLCOLLISION%")
        ...
        noise_weightings = [(float(line[0]), line[1]) for line in
                            noise_weightings_]
        ...
        def put_model_at_position(mod, x, y, modtype, x_cov=0.01, y_cov=0.01,
                                  xy_cov=0):
            """
            ...
            mod_with_cov = setup_covariance(x_cov_str, y_cov_str,
                                            xy_cov_str).join(
        ...
        def expand_allmodels(allmods, mod, modtype, x, y):
            ...
            """
            mod_at_p = put_model_at_position(mod, x * scale_data,
                                             y * scale_data, modtype)
            return allmods + "\n" + mod_at_p
    ...
    def launch_to_csv(which_file, params, conversion_suffix="",
        ...
        conversion_suffix:   Something to append to the output filename
                             So if it is "foo", rand.png becomes
                             randfoo.launch.
        """
    ...
    def csv_to_png(which_file, params, conversion_suffix="",
        ...
        keep_all_noise = params[
            "keep_all_noise"] if "keep_all_noise" in params else True
        ...
        # Note that the indexing may be confusing, pandas has inserted an
        # "index" column,
        ...
        scale_desired = max(total_x_distance, total_y_distance) / (
            len(all_cones) - 1)
        if scale_desired < 0.0001:
            scale_desired = 0.0001  # Clamp scale to allowed values
        if scale_desired > 100:
            scale_desired = 100
        scale_metadata = ConversionTools.deconvert_scale_metadata(
            scale_desired)
        ...
        def get_cone_color(cone_name, cone_direc):
            if cone_name == "yellow":
                ...
                yaw_pixel_value = int(
                    raw_car_location[3] / (2 * math.pi) * 254 + 1)
                if yaw_pixel_value > 255:
                    yaw_pixel_value = 255
                if yaw_pixel_value < 1:
                    yaw_pixel_value = 1
    ...
    @staticmethod
    def csv_to_launch(which_file, params, conversion_suffix="",
                      override_name=None):
        """
        ...
        This contains a lot of duplicated code from csv_to_png and
        png_to_launch, since
        it's an amalgamation of the two to cut out the semi-deprecated png
        format.
        Improvements could be made to link these three functions better to
        reduce loss,
        perhaps by introducing an internal universal data format as an
        intermediary.
        """
        ...
        for bluecone in blue_cones.itertuples():
            ...
            xy_cov = bluecone[7]
            raw_blue.append(
                ("blue", 1.0 * x, 1.0 * y, 0, x_cov, y_cov, xy_cov))
        ...
        for yellowcone in yellow_cones.itertuples():
            ...
            xy_cov = bluecone[7]
            raw_yellow.append(
                ("yellow", 1.0 * x, 1.0 * y, 0, x_cov, y_cov, xy_cov))
        ...
        for orangecone in orange_cones.itertuples():
            ...
            xy_cov = bluecone[7]
            raw_orange.append(
                ("orange", 1.0 * x, 1.0 * y, 0, x_cov, y_cov, xy_cov))
        ...
        #                                   collision data.
        #
        # And the corresponding sdf_ghost_model and
        # sdf_ghost_model_with_collisions
        ...
        def expand_allmodels(allmods, mod, modtype, x, y, x_cov=0.01,
            ...
            and places the model inside sdf_allmodels
            """
            mod_at_p = put_model_at_position(mod, x, y, modtype, x_cov, y_cov,
                                             xy_cov)
            return allmods + "\n" + mod_at_p
        ...
        for cone in all_cones:
            ...
            if name == "active_noise" or (
                    keep_all_noise and name == "inactive_noise"):
