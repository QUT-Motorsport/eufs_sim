...
def convert_points_to_all_positive(xys):
    """
    If the track dips to the negative side of the x or y axes,
    ...
    return (new_xys, int(max_x - max_neg_x) + 2 * padding,
            int(max_y - max_neg_y) + 2 * padding)


def convert_components_to_all_positive(components):
    """
    Like convert_points_to_all_positive, but given a list of components
    rather than points
    ...
    for comp in components:
        ...
        for point in comp[1]:
            (x, y) = point
            plist.append(
                ((x - max_neg_x) + padding, (y - max_neg_y) + padding))
...
def check_if_overlap(points):
    ...
    points = points[:-10]
    # We want to add in the diagonally-connected points, otherwise you can
    # imagine
    # that two tracks moving diagonally opposite could cross eachother
    # inbetween the pixels,
    # fooling our test.
...
def random_choices(list_to_check, weightings):
    ...
    # and then loop through the weightings and check partial sums of
    # weightings.
    # If the partial sum is strictly more than random_value, then we stop
    # and return
    # the element that pushed the sum over the edge.
    # Strict inequalities are necessary to avoid 0-weighted things getting
    # chosen.
    ...
    # This is due to uniform() returning a number inclusive to the bounds,
    # along
    # with strict inequality testing. (which means it is possible random_value
    # equals sum(weightings) and thus won't be considered due to strict
    # inequalities.
    return [x for idx, x in enumerate(list_to_check) if weightings[idx] > 0][
        -1]
