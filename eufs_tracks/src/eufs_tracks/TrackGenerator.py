...
from .LauncherUtilities import (calculate_tangent_angle, calculate_tangent_vector,
                                ...
                                convert_components_to_all_positive, compactify_points,
                                check_if_overlap, random_choices, get_points_from_component_list)
import collections
...
class Parametrization:
    """
    Class that is used to seemlessly store and combine parametrization
    functions
    for the generator.

    Parametrization functions are defined as functions which take a number
    ∈ [0,1]
    and return a point.  It is used to define geometric curves in terms of a
    percentage.
    So if you had a circle parametrization function, and gave it an input
    0.25, it would
...
class TrackGenerator:
    ...
    #        Slalom:                 Cones in a straight line with 7.5 to 12
    #        meter spacing
    #                                [NOTE: can't be generated at this
    #                                point, added later]
    ...
    @staticmethod
    def pass_logger(node_logger):
        TrackGenerator.logger = node_logger
    ...
    def get_presets():
        ...
        Presets contain all the information that parameterizes track
        generation.
    ...
    def get_preset(name):
        ...
        TrackGenerator.logger.warn("No such preset: " + name)
        TrackGenerator.logger.warn("Defaulting to " + TrackGenerator.get_default_preset())
        return TrackGenerator.get_preset(TrackGenerator.get_default_preset())
        ...
        while overlapped or xys == []:
            ...
            if overlapped:
                if TrackGenerator.FAILURE_INFO:
                    TrackGenerator.logger.warn(
                        "Overlap check " + str(failure_count) + " failed")
                failure_count += 1
...
def get_linearized_function(to_linearize):
    """
    Linearizes function according to linearize parameter of linear_micro
    decorator.
    """

    def linearized_function(point_in, tangent_in, normal_in, length,
                            params={}):
...
def get_cone_function(micro_name):
    """
    Returns the cone placement function utilized by the micro_name micro
    generator.
    """
...
def use_micro(micro_tuple):
    ...
    Changes TrackGenerator.previous_micro to the micro, and returns the
    function so it
    can be used.
    """
...
def generate_random_track(start_point):
    ...
    for goal_point in goal_points:
        ...
        (comps, tangent_out, normal_out,
         added_length) = generate_point_to_point(
            point_in,
            point_out,
            tangent_in,
            normal_in,
        ...
        if check_if_overlap(test):
            if TrackGenerator.FAILURE_INFO:
                TrackGenerator.logger.warn("Early Overlap Checking: Failed")
    ...
    # We're gonna set a checkpoint that is close but not exactly the start
    # point
...
def generate_point_to_point(point_in,
                            ...
                            depth=20):
    """
    Generates a track from point_in to point_out with incoming tangent
    tangent_in,
    ...
    depth is the max amout of times it may recurse (this is a recursive
    function that
...
#
# 1: "generic"
#         These have the signature generic_<type>(point_in,tangent_in,
#         normal_in,params)
#         and output (parametrization_out, tangent_out, normal_out,
#         added_length)
#
#         Generics just create a generically random component, with no extra
#         requirements.
#
# 2: "refocus"
#         These have the signature
#                 refocus_<type>(point_in,point_out,tangent_in,normal_in,
#                 params)
...
# 3: "connector"
#         These have the signature:
#                 connector_<type>(point_in,point_out,tangent_in,
#                 tangent_out,normal_in,params)
#         with the same output as generics
#
...
#         with the same output as generics
#
#         If we consider the component to end at "point_out", then linear
#         components
#         guarantee that point_out will lie on the ray emanating from
#         point_in in the
#         direction of tangent_in.  The outgoing tangent will be guaranteed
#         to be
#         the same as tangent_in.
#
#         The length parameter of the signature can be ommited if the
#         "linearize" option
#         of the linear_micro decorator is ommitted, provided that one of
#         the params{}
#         of the component is named "length".
#
# 5: "special"
#         There is no set signature for special micros.  They are identified
#         solely
#         by the presence of the special_micro decorator.  Special micros
#         delegate to
#         a special cone-placement function rather than the normal one.
#
# 6: "rare"
#         Similar to special, rare tells the generator that this specific
#         component
#         is only allowed to appear a couple times.
#
# extra: "parametric"
#         These have variable signatures, but always output a function of
#         one variable
#         which ranges from 0 to 1, parametrizing the curve they represent
#         in terms of it.
#
#         They aren't really micro generators, but are crucial helper
#         functions for them.
#
# extra: "cone"
#         These are used to define cone placements, like cone_default and
#         cone_slalom
#         They always take in a list of points, and return a list of triples
#         (px, py, color)
#         of the (px, py) position of a cone of color color that will be
#         placed by the
...
def cone_default(xys, starting=False, track_width=None, slalom=False,
    ...
    Takes in a list of points, returns a list of triples dictating x,
    y position
    ...
    is useful if accessing ConversionTools.xys_to_png from a non-generator
    context.
    """
    ...
    if prev_points is not None:
        first_orange_cones = [(x, y) for x, y, c in prev_points if c is CONE_ORANGE][:2]
        all_points_south = [(x, y) for x, y, c in prev_points if c is CONE_INNER]
        all_points_south = (all_points_south[-cone_check_amount:] + first_orange_cones)
        all_points_north = [(x, y) for x, y, c in prev_points if c is CONE_OUTER]
        all_points_north = (all_points_north[-cone_check_amount:] + first_orange_cones)
    ...
    for i in range(len(xys)):
        ...
        # Check if normal direction suddenly flipped relative to last
        # iteration.
        ...
        # This is because we want to make sure no cones are too close to
        # eachother,
...
def generic_constant_turn(point_in,
    ...
    # Returns a list of points and the new edge of the racetrack and the
    # change in length
...
def generic_hairpin_turn(point_in,
    ...
    params["uniform_circles"]: if True, all circles have the
                               ...
                               if False, all circles have random radii to
                               add variety.
    ...
    params["wobbliness"]: The percentage of circumference of the circles
                          ...
                          The hairpin would self-intersect with the given
                          wobbliness.
    ...
    # If you draw a diagram, this gives you a diamond with angles pi/2,pi/2,
    # L, and pi/2-L
    # where L is:
    # ((2*pi*(1-wobbliness))/2)
    ...
    # Otherwise we want to cap it there.  So we find the inverse-function
    # for "wobbliness"
    ...
    for a in range(0, switchbacks):
        ...
        (points_out, tangent_out, normal_out,
         delta_length) = generic_constant_turn(
            point_in,
            tangent_in,
            normal_in,
            params={
...
def refocus_constant_turn(point_in,
    ...
    circle_func = Parametrization(
        lambda a: intermediate_point(point_in, center, a))

    # We are going to iteratively walk around the circle until we are facing
    # the right way
    points_out = []
    ...
    for t in range(0, max_range):
        ...
        if t != 0:
            ...
            if abs(
                    cap_angle(appropriate_angle) - cap_angle(
...
def connector_constant_turn(point_in,
                            ...
                            params={}):
    """
    Complicated connector function that attempts to abide by contest
    regulations.
...
def connector_bezier(point_in,
    ...
    params["scale_in"]:  roughly corresponds to how "straight" the start of
    the bezier is.
    params["scale_out"]: roughly corresponds to how "straight" the end of
    the bezier is.

    This function does not accurately keep track of normals and track
    length, so
...
def parametric_bezier(control_points):
    ...
    That is, the result will be a function that takes a time parameter from
    0 to 1
    and traveling along it results in the points on the bezier.
    I made this code match the Bezier curve definition on wikipedia as
    closely as
...
def parametric_straight(slope_vec, start_point, line_length):
    """Returns the parametric function of a line given a slope and start
    point"""
    ...
    return Parametrization(
        lambda t: to_return(slope_vec, start_point, line_length, t))


def parametric_circle(start_point, center_point, delta_angle):
    """
    Returns a function in terms of t ∈ [0,1] to parameterize a circle
