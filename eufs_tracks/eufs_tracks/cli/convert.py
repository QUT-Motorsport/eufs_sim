...
class EUFSTracksConvert(VerbExtension):
    ...
    def main(self, args):
        assert args.fsource in [
            "launch",
            "csv",
        ], "fsource must be either 'launch' or 'csv'"
        assert args.ftarget in [
            "launch",
            "csv",
        ], "ftarget must be either 'launch' or 'csv'"
        ...
        if not os.path.exists(args.track):
            if args.fsource == "csv":
                ...
                args.track = os.path.join(
                    TRACKS_SHARE, "launch", args.track + ".launch"
                )
